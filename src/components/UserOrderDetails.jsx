...
const UserOrderDetails = () => {
  ...
  if (isLoading) {
    return <div className="text-center pt-10">Loading order details...</div>
  }

  if (!data) {
    return <div className="text-center pt-10">Order not found.</div>
  }
    ...
    <div className={`py-4 min-h-screen ${styles.section}`}>
      ...
      {data.cart.map((item, index) => (
        <div key={index} className="w-full flex items-start mb-5">
          ...
          {
            item.isReviewed || item.status !="deliverd" ? null: (
      ...
      {open && (
        <div className='w-full fixed top-0 left-0 h-screen bg-[#0005] z-50 flex items-center justify-center'>
         <div className='w-[50%] h-min bg-[#fff] shadow rounded-md p-3 overflow-y-scroll'>
          ...
          <div className='w-full flex'>
            <img src={`${selectedItem?.images[0]?.url}`}  className='w-[80px] h-[80px]'/>
            <div>
          ...
          <br/>
          {/*  Give a rating*/}
           <h5 className='pl-3 text-[20px] font-[500]'>Give a Rating <span className='text-rose-500'>*</span></h5> 
           ...
           <textarea name="comment" cols="10" rows="5"value={comment} onChange={(e)=>setComment(e.target.value)} placeholder='How was your product? write your expresion about it' className='mt-2 w-[95%] border p-2 outline-none'></textarea>
           </div>
